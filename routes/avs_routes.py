...
def generate_token():
  ...
  if existing_key:
    msg = {
      'error': 'You already have an API key. Please use that instead.',
      'status': 'failure'
    }
  ...
  try:
    ...
    if VALID_ADDRESS:
      ...
      if is_near_match_list:
        ...
        near_match = [addr for addr in near_match_list if fuzz.partial_ratio(client_data['addressLine1'], addr['addressLine1']) >= 30]
        near_match = sorted(near_match, key=lambda x: fuzz.partial_ratio(client_data['addressLine1'], x['addressLine1']), reverse=True)
        ...
        failed_address_recommendation['addressLine2'] = near_match[0].get('addressLine2', None)
      ...
      response = {
        "avsAddressDetails": {
            ...
            "address": client_data,
            "nearMatchAddressRecommendation": failed_address_recommendation or {"msg": "no recommendation for the address submitted"}
        }}
...
def get_list_of_addresses():
  """
    This endpoint retrieves a list of addresses from the database based on the query parameters specified in the request. 
